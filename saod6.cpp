	...
	while (flag) {
		...
		case 5:
			if (head) {
				...
				if (hmax == hmin)
				{
					cout << "Минимальный и максимальный элемент на одной высоте\n";
					break;
				}
				cout << "\n";
				cout << "Максимальный элемент " << hmax->key << " на высоте  " << hmax->heightree <<"\n";
				cout << "Минимальный элемент " << hmin->key << " на высоте   " << hmin->heightree <<"\n";
				cout << "\n";

				while (hmax != hmin) {
					head = remove(head, hmax->key);
					head = remove(head, hmin->key);
					head->heightree = 1;
					counthtree(head);
					hmax = NULL;
					hmin = NULL;
